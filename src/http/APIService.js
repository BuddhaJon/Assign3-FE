...
const API_URL = 'https://BuddhaJon.pythonanywhere.com/'; /* 'http://localhost:8000' http://127.0.0.1:8000/ or  'http://yourPythonAnywhereName.pythonanywhere.com/'*/
  ...
  getStock(stockId) {
    const url = `${API_URL}/api/stocks/${stockId}/`;
    let jwtToken = localStorage.getItem('access');
    const headers = { Authorization: `JWT ${jwtToken}` };
    return axios.get(url, { headers: headers });
  }
